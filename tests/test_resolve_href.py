...
Covers root-relative absolute hrefs (e.g. "/index.html"), which pages served
at a rewritten URL (like /admin, rewritten from core/admin.html) must use
instead of "../"-style relative paths that assume a fixed directory depth.
"""
...
def test_root_relative_absolute_href_resolves_against_site_root_not_filesystem_root():
    page = SITE_ROOT / "core" / "admin.html"
    target = _resolve_href("/index.html", page)
    assert target == (SITE_ROOT / "index.html").resolve()
